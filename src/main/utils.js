...
const UpdateManager = require("./updateManager").default;
...
async function sendDeviceInfo() {
    try {
        ...
        options["Build"] = readBuildVersion()
        options["App-name"] = pjson.name;
        options["Screen-resolutions"] =  await DisplayManager.getAllScreenResolution()
        options["Bluetooth-ID"] =  await readBluetoothID()
        options["Kernel-version"] = osInfo["kernel"]
        const model = await this.executeCommand("cat /proc/cpuinfo | grep 'Model' | awk -F': ' '{print $2}'")
 ...
 */
async function setBluetoothID(id) {
    try {
 ...
 */
async function readBluetoothID() {
    let bluetooth_id;
